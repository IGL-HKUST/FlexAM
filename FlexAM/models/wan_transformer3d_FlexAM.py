...
class WanAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.modulation_density = nn.Parameter(torch.randn(1, 2, dim) / dim**0.5)
    ...
    def forward(
        ...
        Args:
            ...
            density_emb(Tensor): Shape [B, 2, C]
        ...
        density_emb = (self.modulation_density + density_emb).chunk(2, dim=1)

        # self-attention
        temp_x = self.norm1(x) * (1 + e[1]) + e[0] + density_emb[0]
        ...
        def cross_attn_ffn(x, context, context_lens, e):
            ...
            # ffn function
            temp_x = self.norm2(x) * (1 + e[4]) + e[3] + density_emb[1]
            temp_x = temp_x.to(dtype)
...
class Head(nn.Module):
    ...
    def __init__(self, dim, out_dim, patch_size, eps=1e-6):
        ...
        self.modulation_density = nn.Parameter(torch.randn(1, 1, dim) / dim**0.5)

    def forward(self, x, e, density_emb):
        r"""
        Args:
            x(Tensor): Shape [B, L1, C]
            e(Tensor): Shape [B, C]
            density_emb(Tensor): Shape [B, C]
        """
        if e.dim() > 2:
        ...
        density_emb = (self.modulation_density + density_emb.unsqueeze(1)).chunk(1, dim=1)
        x = (self.head(self.norm(x) * (1 + e[1]) + e[0] + density_emb[0]))
        return x
...
class WanTransformer3DModel_FlexAM(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    r"""
    ...
    def __init__(
        ...
        self.density_embedding = nn.Sequential(
            nn.Linear(freq_dim, dim), nn.SiLU(), nn.Linear(dim, dim))
        self.density_projection = nn.Sequential(nn.SiLU(), nn.Linear(dim, dim * 2))
        ...
        if add_cnn_block:
            self.cnn_conv1 = nn.Sequential(
                nn.Conv3d(in_dim_cnn_block, 192, kernel_size=(1, 3, 3), padding=(0, 1, 1)),
                nn.GroupNorm(24, 192),
                nn.SiLU(),
            )
            
            self.cnn_conv2 = nn.Sequential(
                nn.Conv3d(192, 192, kernel_size=(1, 3, 3), padding=(0, 1, 1)),
                nn.GroupNorm(24, 192),
                nn.SiLU(),
            )
            
            self.cnn_conv3 = nn.Sequential(
                nn.Conv3d(192, 96, kernel_size=(1, 3, 3), padding=(0, 1, 1)),
                nn.GroupNorm(12, 96),
                nn.SiLU(),
            )
            
            self.cnn_conv4 = nn.Sequential(
                nn.Conv3d(96, 96, kernel_size=(1, 3, 3), padding=(0, 1, 1)),
                nn.GroupNorm(12, 96),
                nn.SiLU(),
            )
            
            self.cnn_conv5 = nn.Conv3d(96, out_dim_cnn_block, kernel_size=(1, 1, 1))
        else:
            self.cnn_conv1 = None
            self.cnn_conv2 = None
            self.cnn_conv3 = None
    ...
    def forward(
        ...
        Args:
            ...
            additional_control (List[Tensor]):
                List of input multi control videos tensors, each with shape [C_in*5, F, H, W]
            density (Tensor, *optional*):
                Density tensor of shape [B]                
        Returns:
            List[Tensor]:
        ...
        if y is not None:
            if self.cnn_conv1 is not None and additional_control is not None:
                control_latents_0 = y[:,:x.shape[1],:,:,:]
                inpaint_latents = y[:,x.shape[1]:,:,:,:]
                control_latents_1 = additional_control
                cnn_input = torch.cat([control_latents_0, control_latents_1], dim=1)
                x1 = self.cnn_conv1(cnn_input)
                x2 = self.cnn_conv2(x1)
                x2 = x2 + x1
                x3 = self.cnn_conv3(x2)
                x4 = self.cnn_conv4(x3)
                x4 = x4 + x3
                cnn_output = self.cnn_conv5(x4)
                y = torch.cat([cnn_output, inpaint_latents], dim = 1)
        ...
        if density is not None:
            with amp.autocast(dtype=torch.float32):
                density_emb = self.density_embedding(
                    sinusoidal_embedding_1d(self.freq_dim, density).float())
                density_emb0 = self.density_projection(density_emb).unflatten(1, (2, self.dim))
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            def create_custom_forward(module):
                ...
                return custom_forward
            ckpt_kwargs: Dict[str, Any] = {"use_reentrant": False} if is_torch_version(">=", "1.11.0") else {}
            x = torch.utils.checkpoint.checkpoint(create_custom_forward(self.head), x, e, density_emb, **ckpt_kwargs)
        else:
            x = self.head(x, e, density_emb)
    ...
    def init_weights(self):
        ...
        # init density_embedding and density_projection to zero
        if hasattr(self, 'density_embedding') and self.density_embedding is not None:
            for m in self.density_embedding.modules():
                if isinstance(m, nn.Linear):
                    nn.init.zeros_(m.weight)
                    if m.bias is not None:
                        nn.init.zeros_(m.bias)
        
        if hasattr(self, 'density_projection') and self.density_projection is not None:
            for m in self.density_projection.modules():
                if isinstance(m, nn.Linear):
                    nn.init.zeros_(m.weight)
                    if m.bias is not None:
                        nn.init.zeros_(m.bias)
...
class Wan2_2Transformer3DModel_FlexAM(WanTransformer3DModel_FlexAM):
    r"""
    Wan diffusion backbone supporting both text-to-video and image-to-video.
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            add_cnn_block=add_cnn_block,
            in_dim_cnn_block=in_dim_cnn_block,
            out_dim_cnn_block=out_dim_cnn_block, 
        )
