...
from ..models import (AutoencoderKLWan, AutoTokenizer,
                      Wan2_2Transformer3DModel_FlexAM, WanT5EncoderModel)
...
class Wan2_2FunControlPipeline_FlexAM(DiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        transformer: Wan2_2Transformer3DModel_FlexAM,
        transformer_2: Wan2_2Transformer3DModel_FlexAM = None,
    ...
    def __call__(
        ...
        cos_level: int = 4,
        cos_control_videos: Union[dict[int, torch.FloatTensor]] = None,
        density: float = 1.0,        
        ...
        if control_camera_video is not None:
            ...
            if depth_video is not None:
                video_length = depth_video.shape[2]
                ...
                depth_video = rearrange(depth_video, "(b f) c h w -> b c f h w", f=video_length)
                depth_video_latents = self.prepare_control_latents(
                    None,
                    depth_video,
                    batch_size,
                    height,
                ...
                depth_video_latents = torch.zeros_like(latents).to(device, weight_dtype)
            # Process cos_control_videos
            if cos_control_videos is not None and len(cos_control_videos) > 0:
                cos_latents_list = []
                for level in sorted(cos_control_videos.keys()):
                    cos_video = cos_control_videos[level]
                    if cos_video is not None:
                        cos_video_length = cos_video.shape[2]
                        cos_video = self.image_processor.preprocess(rearrange(cos_video, "b c f h w -> (b f) c h w"), height=height, width=width)
                        cos_video = cos_video.to(dtype=torch.float32)
                        cos_video = rearrange(cos_video, "(b f) c h w -> b c f h w", f=cos_video_length)
                        
                        cos_video_latents = self.prepare_control_latents(
                            None,
                            cos_video,
                            batch_size,
                            height,
                        ...
                        cos_video_latents = torch.zeros_like(latents).to(device, weight_dtype)
                    cos_latents_list.append(cos_video_latents)
                
            # Concatenate all cos latents in channel dimension
            addtional_control_video_latents = None
            if cos_latents_list:
                cos_control_latents = torch.cat(cos_latents_list, dim=1)
                addtional_control_video_latents = torch.cat([depth_video_latents, cos_control_latents], dim=1)
        else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if control_camera_video is not None:
                    ...
                    if addtional_control_video_latents is not None:
                        addtional_control_video_latents_input = (torch.cat([addtional_control_video_latents] * 2) if do_classifier_free_guidance else addtional_control_video_latents).to(device, weight_dtype)
                ...
                density_input = density.expand(latent_model_input.shape[0])
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=device):
                    noise_pred = local_transformer(
                        ...
                        full_ref=full_ref,
                        additional_control=addtional_control_video_latents_input,
                    )
